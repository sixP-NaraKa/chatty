...
export class AppController {
  ...
  async getChatroomsForUserWithParticipantsExceptSelf(
    ...
    return await this.appService.getChatroomsForUserWithParticipantsExceptSelf(
      userId,
    );
  }
  ...
  async getSingleChatroomForUserWithParticipantsExceptSelf(
    ...
    return await this.appService.getSingleChatroomForUserWithParticipantsExceptSelf(
      userId,
      chatroomId,
    );
  }
  ...
  async getSingleChatroomforUserByUserAndParticipantUserId(
    ...
    return await this.appService.getSingleChatroomForUserWithParticipantUserId(
      userId,
      participantUserId,
    );
  }
  ...
  async createChatroomWithParticipants(
    ...
    return await this.appService.createChatroomWithParticipants(
      userId,
      participantUserIds,
      isGroup,
      groupName,
  ...
  async removeUserFromGroupChat(
    @Body() body: { userId: number; chatroomId: number },
  ): Promise<number> {
    return await this.appService.removeUserFromGroupChat(
      body.userId,
      body.chatroomId,
    );
  }
  ...
  async getMessagesForChatroom(
    ...
    return await this.appService.getAllChatMessagesByChatroomId(
      chatroomId,
      oldCursor,
    );
  }
  ...
  async insertMessage(
    ...
    return await this.appService.insertMessage(
      body.message,
      body.userId,
      body.chatroomId,
    );
  ...
  async deleteMessage(
    @Body() body: { messageId: number; userId: number; chatroomId: number },
  ): Promise<boolean> {
  ...
  async getImageMessage(
    ...
    const file = fs.createReadStream(
      `${this.imageFilesFolder}/${imageId}.png`,
      { autoClose: true },
    );
    file.on('error', () =>
  ...
  async insertFileMessage(
    ...
    const m = await this.appService.insertFileMessage(
      file.originalname,
      uuid,
      userId,
      chatroomId,
  ...
  async getFileMessage(
    ...
    file.on('error', () =>
      console.log('could not read file', fileId, 'as it does not exist'),
    );
    return new StreamableFile(file);
  }
  ...
  async insertEmoteReaction(
    ...
    return await this.appService.insertEmoteReaction(
      body.messageId,
      body.userId,
      body.emoteId,
    );
