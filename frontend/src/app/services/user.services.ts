...
export class UserService {
    ...
    getSingleChatroomForUserWithUserIdAndParticipantUserId(participantUserId: number): Observable<ChatRoomWithParticipantsExceptSelf> {
        return this.http.get<ChatRoomWithParticipantsExceptSelf>(`${this.backendHost}/api/user/chatroom/1on1?participant_user_id=${participantUserId}`)
    }

    getSingleChatroomForUserWithParticipantsExceptSelf(chatroomId: number) {
        return this.http.get<ChatRoomWithParticipantsExceptSelf>(`${this.backendHost}/api/user/chatroom?chatroomId=${chatroomId}`);
    }

    createChatroom(participantUserIds: number | number[], is_group: boolean = false, groupName?: string | null) {
        return this.http.get<ChatRoomWithParticipantsExceptSelf>(`${this.backendHost}/api/user/chatrooms/create?participant_user_id=${participantUserIds}&is_group=${is_group}&group_name=${groupName}`);
    }

    getChatroomMessages(chatroomId: number, oldCursor: number | undefined | null): Observable<[ChatMessageWithUser[], number]> {
        return this.http.get<[ChatMessageWithUser[], number]>(`${this.backendHost}/api/chat/chatmessages?chatroomId=${chatroomId}&oldCursor=${oldCursor}`);
    }
    ...
    sendMessage(message: string, chatroomId: number): Observable<ChatMessageWithUser> {
        return this.http.post<ChatMessageWithUser>(`${this.backendHost}/api/chat/create/chatmessage`, { message: message, userId: this.currentUser.userId, chatroomId: chatroomId });
    }

    sendImageMessage(chatroomId: number, image: File): Observable<ChatMessageWithUser> {
        const formData = new FormData();
        formData.append("image", image);
        return this.http.post<ChatMessageWithUser>(`${this.backendHost}/api/chat/create/chatimagemessage?chatroomId=${chatroomId}`, formData);
    }

    sendFileMessage(chatroomId: number, file: File): Observable<ChatMessageWithUser> {
        const formData = new FormData();
        formData.append("file", file);
        return this.http.post<ChatMessageWithUser>(`${this.backendHost}/api/chat/create/chatfilemessage?chatroomId=${chatroomId}`, formData);
    }

    sendEmoteReaction(messageId: number, emoteId: number) {
        return this.http.post<MessageReaction>(`${this.backendHost}/api/chat/create/chatmessage/reaction`, { messageId: messageId, userId: this.currentUser.userId, emoteId: emoteId });
    }

    removeUserFromGroupChat(userIdToRemove: number, chatroomId: number): Observable<number> {
        return this.http.post<number>(`${this.backendHost}/api/user/chatrooms/groups/remove`, { userId: userIdToRemove, chatroomId: chatroomId });
    }

    addUsersToGroupChat(userIdsToAdd: number | number[], chatroomId: number): Observable<void> {
        return this.http.get<void>(`${this.backendHost}/api/user/chatrooms/groups/add?userIdsToAdd=${userIdsToAdd}&chatroomId=${chatroomId}`);
    }
    ...
    validateFileType(blob: Blob): Observable<[boolean, { ext: any, mime: any } | null]> {
        ...
        return this.http.post<[boolean, { ext: any, mime: any } | null]>(`${this.backendHost}/api/file/validate`, formData);
    }
